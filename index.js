...
function indekseGoreFenomen(fenomens, i) {
  return `${i}. indekste bulunan fenomen: ${fenomens[i].profile}`;
};
...
function fenomenSil(fenomens, i) {

const newFenomens = [ ...fenomens];

newFenomens.splice(i,1);
...
function fenomenEkle(fenomens, number, profile, followers, posts, platform) {
...
function platformaGoreCokGonderiYapanFenomen(fenomens, platform){
  const platformaGore = fenomens.filter(
    (fenomen) => fenomen.platform === platform
    );

  const arananFenomen = platformaGore.reduce(
    (enYuksekPostuOlanFenomen, fenomen) => {
